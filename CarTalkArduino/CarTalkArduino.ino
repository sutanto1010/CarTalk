/*
 Name:		CarTalkArduino.ino
 Created:	11/4/2017 9:10:19 AM
 Author:	Sutanto
*/
...
void loop()
{
	...
	if (Serial.available() > 0)
	{
		...
		if (character == '\n') {
			...
			{
				Log("Character: ", text[i]);
				theText+= text[i];
			}
			//theText[counter] = '\n';
			counter = 0;
			Serial.println(theText);
			scrollText(string2char(theText));
		}
	}
}


void Log(char* text, int intValue) {
	Serial.print(text);
	Serial.println(intValue);
}

//Source: https://coderwall.com/p/zfmwsg/arduino-string-to-char
